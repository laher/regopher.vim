...
" into the statusline.
function! regopher#statusline#Show() abort
  " lazy initialiation of the cleaner
  if !s:timer_id
    " clean every 60 seconds all statuses
    let interval = regopher#config#StatuslineDuration()
    let s:timer_id = timer_start(interval, function('regopher#statusline#Clear'), {'repeat': -1})
  endif
...
function! regopher#statusline#Update(status_dir, status) abort
  ...
  call regopher#statusline#Clear(0)
...
function! regopher#statusline#Clear(timer_id) abort
