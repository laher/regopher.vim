...
function! regopher#ui#OpenWindow(title, content, filetype) abort
  " Ensure there's only one return window in this session/tabpage
  call regopher#util#Windo("unlet! w:regopher_return_window")
  " Mark the window we're leaving as such
  let w:regopher_return_window = 1
...
function! regopher#ui#GetReturnWindow() abort
  for l:wn in range(1, winnr("$"))
    if !empty(getwinvar(l:wn, "regopher_return_window"))
      return l:wn
    endif
  endfor
endfunction
...
function! regopher#ui#CloseWindow() abort
  ...
  let l:rw = regopher#ui#GetReturnWindow()
  if !empty(l:rw)
    execute l:rw . 'wincmd w'
    unlet! w:regopher_return_window
  endif
endfunction

" OpenDefinition parses the current line and jumps to it by openening a new
" tab
function! regopher#ui#OpenDefinition(filter) abort
  let curline = getline('.')
