" Spawn starts an asynchronous job. See the description of regopher#job#Options to
" understand the args parameter.
"
" Spawn returns a job.
function! regopher#job#Spawn(cmd, args)
  let l:options = regopher#job#Options(a:args)
  return regopher#job#Start(a:cmd, l:options)
endfunction
...
"     See statusline.vim.
"   'for':
"     The g:regopher_list_type_command key to use to get the error list type to use.
...
function! regopher#job#Options(args)
  let cbs = {}
  let state = {
  ...
  function state.show_status(job, exit_status) dict
    ...
    if regopher#config#EchoCommandInfo()
      let prefix = '[' . self.statustype . '] '
      if a:exit_status == 0
        call regopher#util#EchoSuccess(prefix . "SUCCESS")
      else
        call regopher#util#EchoError(prefix . "FAIL")
      endif
    endif
    ...
    call regopher#statusline#Update(self.jobdir, status)
  endfunction
  ...
  function! s:start(args) dict
    if regopher#config#EchoCommandInfo() && self.statustype != ""
      let prefix = '[' . self.statustype . '] '
      call regopher#util#EchoSuccess(prefix . "dispatched")
    endif
  ...
  function state.show_errors(job, exit_status, data)
    ...
    let l:listtype = regopher#list#Type(self.for)
    if a:exit_status == 0
    ...
    try
      ...
      call regopher#list#ParseFormat(l:listtype, self.errorformat, out, self.for)
    ...
    if empty(errors)
      ...
      call regopher#util#EchoError([self.dir] + self.messages)
      call win_gotoid(l:winid)
      return
    endif
...
" regopher#job#Start runs a job. The options are expected to be the options
" suitable for Vim8 jobs. When called from Neovim, Vim8 options will be
" transformed to their Neovim equivalents.
function! regopher#job#Start(cmd, options)
  let l:cd = exists('*haslocaldir') && haslocaldir() ? 'lcd' : 'cd'
  ...
  if regopher#util#HasDebug('shell-commands')
    call regopher#util#EchoInfo('job command: ' . string(a:cmd))
  endif
...
function! regopher#job#Stop(job) abort
  if has('nvim')
    call jobstop(a:job)
    return
  endif
  ...
  call job_stop(a:job)
  call regopher#job#Wait(a:job)
  return
endfunction

function! regopher#job#Wait(job) abort
  if has('nvim')
    call jobwait(a:job)
    return
  endif
